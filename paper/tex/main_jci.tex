...
the deployer's preferred action by its value gap, and Theorem~ef{thm:info} shows
that some mixing of this kind cannot be avoided by any design that learns the
population effect uniformly. Hadad et al.~\cite{hadad2021confidence} build confidence
intervals for policy values from bandit data when assignment probabilities decay at a
polynomial rate. Under a sharpening policy the probabilities decay exponentially in
