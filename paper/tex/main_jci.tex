...
Our main result is that the two targets have very different prices. With a single
temperature that shrinks at a suitable rate, the boundary effect can be estimated
...
fixed amount, boundary inference under a common temperature is expensive as well
...
The tension between reward and estimation has mostly been studied for bandits
without contexts~\cite{erraqabi2017trading,simchilevi2023multi}. Liang and
Bojinov~\cite{liang2023mixture} mix any bandit algorithm with a Bernoulli design
whose weight decays slowly, which keeps anytime-valid inference on the average
effect; this is essentially the uniform mixing we recommend. Other work covers
inference when propensities decay~\cite{hadad2021confidence}, adaptive designs for
effect estimation and for policy choice~\cite{kato2020efficient,kasy2021adaptive},
and their use in political science~\cite{offerwestort2021adaptive}. Closer to our
notion of cost are conservative bandits~\cite{wu2016conservative}, safe exploration
for policy evaluation~\cite{wan2022safe}, the design of logging policies for
off-policy evaluation~\cite{tucker2023variance,douglas2026logging} and the balance
between regret and effect estimation studied by Koppel et
al.~\cite{koppel2025explore}. The allocation $q\propto g^{-1/2}$ is optimum
...
$O_p((n\tau_n)^{-1/2}+\tau_n)$; we have not tried to show that this rate is optimal.
...
The sample size does not appear in \eqref{eq:info}, so a larger deployment does
not make the population effect cheaper to learn. At a fixed precision $\delta$, the
inequality gives
...
the temperature is raised everywhere, which is expensive near the boundary.
...
Logs from a sharpening policy are good for one question and poor for another. If we
want to know how outcomes respond where the policy is indifferent, and deviations cost
little there, a growing deployment gives increasingly precise inference while the
exploration loss vanishes. If we want the population average effect, the worst-case
loss must grow without bound, and a larger deployment does not help. Anyone reporting
an effect estimated from such logs should therefore say which of the two it is.
...
policy's confidence, so reaching confident contexts requires a temperature that is
expensive near the boundary. Uniform mixing, context-specific temperatures and
