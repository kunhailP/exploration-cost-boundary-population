...
error. Hatched regions, bounded by white curves, have root mean
...
deviation of the Horvitz--Thompson estimator and of an oracle augmented
estimator that uses the true regression (a reference, not an estimator used
in the simulation), both for $\theta$, and the first-order approximation to
the root mean squared error of the boundary estimator of $\beta_0$. Markers:
Monte Carlo root mean squared error of the Horvitz--Thompson estimator, of the
augmented estimator with a fitted linear working model, and of the boundary
estimator. (b) Wald coverage with 95\% Monte Carlo
...
number of off-greedy actions; lines are exact expectations, markers Monte
Carlo.}
\label{fig:sweep}
\end{figure}
