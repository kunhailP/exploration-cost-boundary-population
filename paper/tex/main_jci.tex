...
That local and population effects differ, and that limited overlap makes
average effects hard to estimate, are familiar facts. What this paper adds is a
comparison of the two targets inside one outcome model and on one cost scale:
the boundary effect admits valid inference while the cumulative cost of
exploration tends to zero, whereas no estimator learns the population effect
uniformly without divergent cost. Estimability becomes a statement about what
an experiment must pay for.
...
separation in computed risks for a simple design.
...
strategies given the conversation so far, $H$; $\Delta(H)$ is the difference of
the two scores and the system logs the probability $e(H)$ with which it chose
evidence. The operational gap $g(H)$ is the loss in the deployer's own
objective, for example predicted engagement, from using the lower-scoring
strategy. The outcome $Y$ is the person's attitude after the conversation,
measured separately and not used by the policy. The generator that writes the
message under each strategy is held fixed. The theory below does not use
anything specific to language models beyond this structure.
...
Take a one-dimensional score, $\Delta(h)=h$, and outcome laws whose mean
functions are bounded and Lipschitz near the decision boundary
(Assumption~\ref{ass:D}). Let the logs come from one common temperature
$\tau_n=n^{-\zeta}$ with $\zeta\in(1/2,1)$.
\begin{enumerate}[label=(\roman*),leftmargin=2em,itemsep=2pt]
\item The boundary effect $\beta_0$ has an asymptotically valid Wald interval,
and the cumulative exploration loss $R_n$ tends to zero
(Theorem~\ref{thm:boundary}).
\item In the same model, outcome laws with equal boundary effects but different
population effects cannot be distinguished whenever $R_n\to0$, and uniformly
consistent estimation of $\theta$ requires divergent worst-case loss
(Theorem~\ref{thm:separation}).
\item Without extrapolation structure, precision $\delta$ for $\theta$, uniformly
over the model, requires worst-case loss of order at least
$(\E\sqrt g)^2/\delta^2$, whichever design and estimator are used, and this does
not decrease with $n$ (Theorem~\ref{thm:info}).
\end{enumerate}
Sections~\ref{sec:boundary}--\ref{sec:separation} state these results
precisely, and Section~\ref{sec:temperature} draws the consequences for
temperature as a design lever.

Figure~\ref{fig:map} illustrates the separation for the design used in
Section~\ref{sec:sims}, with risks computed by numerical integration: the exact
risk of the Horvitz--Thompson estimator of $\theta$, which is unbiased, and a
first-order approximation to the root mean squared error of the boundary
estimator, $\{(\beta_{\ov,\tau}-\beta_0)^2+V_1/n\}^{1/2}$, where $V_1$ is its
linearization variance from the proof of Theorem~\ref{thm:boundary}. Across
\foCells{} Monte Carlo checks with $n\tau\ge3$ (\foReps{} replications each),
the Monte Carlo root mean squared error is between \foRatioMin{} and
\foRatioMax{} times the approximation; at $n\tau=\foNtauTiny$ the ratio is
\foRatioTiny{}, so the approximation overstates the error there. Under one
...
\caption{Root mean squared error (colour, $\log_{10}$ scale) under one
common temperature $\tau$ for the design of Section~\ref{sec:sims}: $H\sim
U(-1,1)$, $\Delta(h)=h$, $g(h)=|h|$, $c(h)=1+|h|$, unit noise. (a) Exact
root mean squared error of the Horvitz--Thompson estimator of the population
effect $\theta$. (b) First-order approximation, including the exact bias, to
the root mean squared error of the estimator of Theorem~\ref{thm:boundary} for
the boundary effect $\beta_0$; the shaded region below the dotted line has
$n\tau<3$, where Monte Carlo checks show the approximation overstates the
error. White curves enclose the regions with root mean
...
Assumption (D5) is what makes boundary exploration cheap: the deployer's
objective is nearly indifferent near the boundary, so choosing the unpreferred
strategy there costs little. It does not describe a fixed cost per exploratory
action, such as a review or payment incurred whenever the policy deviates. If
every off-greedy action costs at least $c_0>0$, then
$R_n\ge c_0\,n\E[q_{\tau_n}(H)]=2\log2\,c_0f(0)\,n\tau_n(1+o(1))$, which diverges
under the condition $n\tau_n\to\infty$ of Theorem~\ref{thm:boundary}. Under such
costs, boundary inference also requires divergent cumulative loss, and the
separation below does not hold in this form.
...
Figure~\ref{fig:costs} shows the allocation costs across deployment sizes at
$V_{\rm sp}=0.1^2$; Figure~\ref{fig:costs-small} in Appendix~\ref{app:figures}
repeats it at $V_{\rm sp}=0.03^2$ with the same ordering. In a
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figs/fig3_exploration_costs.pdf}
\caption{Cumulative exploration loss as deployment size increases, from
closed-form calculations with $H\sim U(-1,1)$, $g(h)=|h|$, $\sigma^2=1$ and
the sparse-exploration criterion fixed at $V_{\rm sp}=0.1^2$. Designs are
matched on this criterion, not on realized finite-sample error. A
context-dependent temperature implements the gap-based probabilities and gives
the same curve.}
\label{fig:costs}
\end{figure}
...
estimator with a fitted linear working model) at $1/\tau=\curveInvTauMax$. Over the same range, coverage of the
boundary effect rises from \curveBndCovLo{} to \curveBndCovHi{}, and the
exploration loss falls from \curveLossLo{} to \curveLossHi{}. Boundary coverage
improves over this range because $n\tau$ stays at least \curveNtauMin{}. This is
not a monotone property of sharpening: at fixed $n$, lowering the temperature
further drives $n\tau$ toward zero, and the boundary interval then fails as
well, as Theorem~\ref{thm:boundary} requires $n\tau_n\to\infty$ and
Table~\ref{tab:boundary} shows at $n\tau\approx\smallNtau$.

Error diagnostics for the same runs are in Figure~\ref{fig:sweep-error} in
Appendix~\ref{app:figures}. The Monte Carlo
error of the Horvitz--Thompson estimator understates its exact standard
deviation once the policy is sharp: at $1/\tau=\curveInvTauMax$ the Monte Carlo root mean
...
Horvitz--Thompson estimator, the exact standard deviation rather than the Monte
Carlo points describes the error. The oracle augmented estimator, which uses
the true regression, is shown only as an exact reference; its variance is not
the variance of the fitted augmented estimator whose Monte Carlo results are
plotted. For the boundary estimator the line is the first-order approximation
described in Section~\ref{sec:separation}, and the Monte Carlo points agree
with it.
...
per temperature, same design as Figure~\ref{fig:map}. (a) Wald coverage with
95\% Monte Carlo intervals for the population effect (Horvitz--Thompson and
augmented estimator with a fitted linear working model) and for the boundary
effect; the top axis shows $n\tau$. Boundary coverage is low while $n\tau^3$
is large, as Theorem~\ref{thm:boundary}(iv) requires. (b) Cumulative
exploration loss $R_n$ and expected number of off-greedy actions; lines are
exact expectations, markers Monte Carlo.}
\label{fig:sweep}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figs/figA1_exploration_costs_small_delta.pdf}
\caption{As Figure~\ref{fig:costs}, with the sparse-exploration criterion fixed
at $V_{\rm sp}=0.03^2$.}
...
estimator that uses the true regression (a reference, not an estimator used in
the simulation), both for $\theta$, and the first-order approximation to the
root mean squared error of the boundary estimator of $\beta_0$. Markers: Monte
Carlo root mean squared error of the Horvitz--Thompson estimator, the augmented
estimator with a fitted linear working model, and the boundary estimator.}
